...
def take_a_step(path, found_paths, e_v):
    ...
    for nb in path[-1].neighbors:
        if nb.name == 'end':
            ...
            found_paths = take_a_step(path + [nb], found_paths, e_v)
        elif nb.size == 'small':
            if nb not in path:
                found_paths = take_a_step(path + [nb], found_paths, e_v)
            elif nb in path and e_v:
                e_v = False
                found_paths = take_a_step(path + [nb], found_paths, e_v)
            else:
                pass
...
def find_path(file_path, extra_visit):
    ...
    found_paths = take_a_step(path, found_paths, extra_visit)
